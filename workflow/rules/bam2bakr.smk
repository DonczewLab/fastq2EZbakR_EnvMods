...
if NORMALIZE:
    ...
    rule normalize:
        ...
        shell:
            r"""
            set -euo pipefail
            mkdir -p results/normalization logs/normalize
    ...
    rule normalize:
        ...
        shell:
            ...
            mkdir -p results/normalization logs/normalize
            touch {output}
            echo "NORMALIZE=False; scale file is placeholder" > {log}
            """
...
if not config.get("lowRAM", False):
    ...
    rule merge_features_and_muts:
        ...
        shell:
            r"""
            set -euo pipefail
            mkdir -p results/merge_features_and_muts results/merge_features_and_muts/duckdb results/arrow_dataset logs/merge_features_and_muts
            chmod +x {params.rscript}
...
rule makecU:
    ...
    shell:
        r"""
        set -euo pipefail
        mkdir -p results/cB logs/makecU
        chmod +x {params.shellscript}
...
rule makecB:
    ...
    shell:
        r"""
        set -euo pipefail
        mkdir -p results/cB logs/makecB
...
rule makecUP:
    ...
    shell:
        r"""
        set -euo pipefail
        mkdir -p results/cUP logs/makecUP
...
rule maketdf:
    ...
    envmodules:
        ...
        config["modules"]["ucsc"],
        config["modules"]["igvtools"],
        config["modules"]["star"],
        config["modules"]["java"],
    shell:
        r"""
        set -euo pipefail
        mkdir -p results/tracks logs/maketdf
        chmod +x {params.shellscript}
        chmod +x {params.pythonscript}
