...
def mods(*names):
    """
    Return a list of module strings from config/modules.yaml in the order provided.
    Usage: envmodules: mods("python", "fastp")
    """
    return [config["modules"][n] for n in names]
...
MULTIQC_INPUT = expand(
    "results/fastqc/{sample}.{read}_fastqc.{ext}",
    sample=SAMP_NAMES,
    read=READS,
    ext=["html", "zip"],
)
