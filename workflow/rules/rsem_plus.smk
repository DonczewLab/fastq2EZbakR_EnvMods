...
rule rsem_to_csv:
    ...
    shell:
        r"""
        set -euo pipefail
        mkdir -p results/rsem_csv logs/rsem_to_csv
...
# Estimate transcript isoform fraction new estimates
rule transcript_fn:
    input:
    ...
    shell:
        r"""
        set -euo pipefail
        mkdir -p results/transcript_fn logs/transcript_fn
...
rule combine_fn:
    ...
    shell:
        r"""
        set -euo pipefail
        mkdir -p results/transcript_fn logs/combine_fn
        : > {output}
        cat {input} >> {output}
        echo "combined $(echo {input} | wc -w) files" > {log}
        """
